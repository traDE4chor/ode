...
import io.swagger.trade.client.jersey.auth.ApiKeyAuth;
import io.swagger.trade.client.jersey.auth.Authentication;
import io.swagger.trade.client.jersey.auth.HttpBasicAuth;
import io.swagger.trade.client.jersey.auth.OAuth;
...
public class ApiClient {
	...
	public List<Pair> parameterToPairs(String collectionFormat, String name,
		...
		String format = (collectionFormat == null || collectionFormat.isEmpty() ? "csv"
				: collectionFormat);
		...
		if (contentType.startsWith("multipart/form-data")) {
			...
			for (Entry<String, Object> param : formParams.entrySet()) {
				if (param.getValue() instanceof File) {
					...
					FormDataContentDisposition contentDisp = FormDataContentDisposition
							.name(param.getKey()).fileName(file.getName())
							.size(file.length()).build();
					multiPart.bodyPart(new FormDataBodyPart(contentDisp, file,
							MediaType.APPLICATION_OCTET_STREAM_TYPE));
				} else {
					FormDataContentDisposition contentDisp = FormDataContentDisposition
							.name(param.getKey()).build();
					multiPart.bodyPart(new FormDataBodyPart(contentDisp,
							parameterToString(param.getValue())));
				}
			}
			entity = Entity.entity(multiPart,
	...
	public File downloadFileFromResponse(Response response) throws ApiException {
		try {
			...
			IOUtils.copy(response.readEntity(InputStream.class),
					new FileWriter(file));
			return file;
		} catch (IOException e) {
	...
	public File prepareDownloadFile(Response response) throws IOException {
		...
		String contentDisposition = (String) response.getHeaders().getFirst(
				"Content-Disposition");
	...
	public <T> T invokeAPI(String path, String method, List<Pair> queryParams,
		...
		for (Entry<String, String> entry : headerParams.entrySet()) {
			...
			if (value != null) {
				invocationBuilder = invocationBuilder.header(entry.getKey(),
						value);
			}
		}
		...
		if ("GET".equals(method)) {
			...
			response = invocationBuilder.header("X-HTTP-Method-Override",
					"PATCH").post(entity);
		} else {
	...
	private Client buildHttpClient(boolean debugging) {
		...
		// @hahnml: Fix for warnings about restricted headers, check if there is another solution...
