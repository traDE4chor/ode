...
public class OProcess extends OBase {
	...
	public boolean hasMustUnderstandExtension(String namespaceURI) {
		...
		// Check if an extension implementation is registered for the given namespareURI
		// that the
		// engine must understand
		Iterator<OExtension> iter = mustUnderstandExtensions.iterator();
		while (!found && iter.hasNext()) {
			OExtension ext = iter.next();
			if (ext.namespaceURI.equals(namespaceURI)) {
				found = true;
			}
		}
